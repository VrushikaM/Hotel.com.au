namespace HotelAPI.Common.Cache
{
	public class CacheKeyBuilder
	{
		...
		public static string HotelsByGeoNode(string geoNodeType, int geoNodeId, string? searchTerm, int maxCount)
			=> $"hotel:list:{geoNodeType}:{geoNodeId}:{(string.IsNullOrWhiteSpace(searchTerm) ? "all" : searchTerm.Trim().ToLowerInvariant())}:{maxCount}";
